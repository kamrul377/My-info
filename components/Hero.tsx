...
import { Mail, Rss, SquarePen } from "lucide-react";
            ...
            <div className="max-w-6xl mx-auto grid lg:grid-cols-12 gap-12 items-center">
                ...
                >
                    ...
                    {/* <div className="flex flex-wrap justify-center lg:justify-start items-center gap-4 pt-4">
                        ...
                        <Link
                            href="/blog"
                            className="px-5 ml-12 py-1.5  text-slate-900 rounded-md shadow-md hover:opacity-95 transition border outline-1 border-indigo-800 dark:text-blue-200"
                        >
                            Blog Feed
                        </Link>
                    ...
                    <div className="flex flex-wrap justify-center lg:justify-start items-center gap-4 pt-4">
                        ...
                        <a
                            href={`mailto:${personalInfo.email}`}
                            className="flex items-center bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white font-medium px-5 py-3 rounded-lg shadow-sm hover:shadow transition-all group font-body text-sm touch-manipulation active:scale-95"
                        >
                            <Mail size={18} className="mr-2 group-hover:scale-110 transition-transform pointer-events-none" /> Contact Me
                        </a>

                        {/* 🌐 Social Links Wrapper (GitHub, LinkedIn, Blog) */}
                        <div className="flex items-center space-x-4 border-l border-slate-200 dark:border-slate-800 pl-4 h-10">

                            {/* GitHub */}
                            <a href={personalInfo.github} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors touch-manipulation" aria-label="GitHub">
                                <svg className="h-6 w-6 fill-current pointer-events-none" viewBox="0 0 24 24"><path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" /></svg>
                            </a>

                            {/* LinkedIn */}
                            <a href={personalInfo.linkedin} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors touch-manipulation" aria-label="LinkedIn">
                                <svg className="h-6 w-6 fill-current pointer-events-none" viewBox="0 0 24 24"><path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" /></svg>
                            </a>

                            {/* 📝 Blog Feed - Just Icon Link */}
                            <Link
                                href="/blog"
                                className="group touch-manipulation flex items-center justify-center p-1"
                                aria-label="Blog Feed"
                            >
                                <SquarePen
                                    size={24}
                                    className="text-slate-400 !transition-all !duration-300 group-hover:!text-indigo-600 dark:group-hover:!text-indigo-400 group-hover:scale-110 pointer-events-none"
                                />
                            </Link>
