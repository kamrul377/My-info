...
// import { Heart, MessageCircle, Share2, Send, ArrowLeft, LogOut, Image as ImageIcon, Plus, Trash2 } from "lucide-react";
...
//     shares: number;
//     hasLiked: boolean;
//     images: string[]; // 👈 মাল্টিপল ইমেজ অ্যারে
...
//     // ফেসবুকের মতো মাল্টিপল ইমেজ লিংক ইনপুটের জন্য স্টেট
...
//     // ইমেজ ইনপুট ডাইনামিকালি হ্যান্ডেল করা
...
//     // 📝 নতুন পোস্ট সাবমিট লজিক (With Images)
//     const handleCreatePost = async () => {
//         if (!newPostText.trim() || !session?.user?.name) return;

//         // খালি ইনপুটগুলো ফিল্টার করে বাদ দেওয়া
...
//     // 🛑 স্ক্রিন ১: মাস্ট লগইন ইউজার ইন্টারফেস (গুগল মেইল গেটওয়ে)
//     if (!session) {
//         return (
...
//                         <h2 className="text-xl font-black font-heading tracking-tight">Login Required</h2>
//                         <p className="text-xs text-slate-400">You must sign up or sign in using a verified Google Mail (@gmail.com) account to enter the social stream.</p>
//                     </div>
//                     <button
...
//     // 🔓 স্ক্রিন ২: মেইন ফেসবুক স্টাইল সোশ্যাল ফিড
//     return (
...
//                         <h1 className="text-xs font-black tracking-widest text-blue-600 dark:text-blue-400 font-heading"> TECH FEED</h1>
...
//                 {/* 📦 ফেসবুক স্টাইল পোস্ট মেকার উইজেট (With Multi-Image support) */}
...
//                     {/* 🖼️ ডাইনামিক ইমেজ ইউআরএল ইনপুট ফিল্ডস */}
//                     <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
//                         <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1"><ImageIcon size={12} /> Attach Image Links</div>
//                         {imageUrls.map((url, idx) => (
//                             <div key={idx} className="flex items-center gap-2">
//                                 <input
//                                     type="text"
//                                     placeholder="Paste Unsplash / online image URL..."
//                                     value={url}
//                                     onChange={(e) => handleImageUrlChange(idx, e.target.value)}
//                                     className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none"
//                                 />
//                                 {imageUrls.length > 1 && (
...
//                         <button onClick={handleCreatePost} className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 py-1.5 rounded-xl text-xs transition">Post Status</button>
//                     </div>
//                 </div>
...
//                             {/* 🖼️ ফেসবুক স্টাইল ইমেজ গ্রিড লেআউট (সিঙ্গেল এবং মাল্টিপল ইমেজ রেন্ডারিং) */}
...
// import { useState, useEffect } from "react";
// import { useSession, signIn, signOut } from "next-auth/react";
// import { Heart, MessageCircle, Share2, Send, ArrowLeft, LogOut, Image as ImageIcon, Plus, Trash2, Loader2 } from "lucide-react";
...
//     const [imageUrls, setImageUrls] = useState<string[]>([""]);
//     const [commentInputs, setCommentInputs] = useState<{ [key: number]: string }>({});
//     const [isUploading, setIsUploading] = useState(false); // 👈 আপলোডিং স্পিনারের জন্য
...
//     // 📂 লোকাল পিসি/মোবাইল থেকে ইমেজ আপলোড হ্যান্ডেলার
...
//         try {
//             // ImgBB এর ফ্রি API ব্যবহার করে ইমেজ হোস্টিং করা হচ্ছে
...
//             if (result.success) {
//                 const uploadedUrl = result.data.url; // 👈 জেনারেট হওয়া অনলাইন ইমেজ লিঙ্ক
//                 const updated = [...imageUrls];
...
//                 {/* 📦 ফেসবুক স্টাইল পোস্ট মেকার উইজেট */}
//                 <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4">
//                     <div className="flex space-x-3 items-start">
//                         <img src={session.user?.image || "/favicon.ico"} alt="" className="h-9 w-9 rounded-full object-cover shrink-0" />
//                         <textarea
...
//                     {/* 🖼️ ডাইনামিক লোকাল আপলোড ও ইমেজ লিংক ফিল্ড */}
//                     <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
...
//                                     <label
//                                         htmlFor={`file-${idx}`}
//                                         className="bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 px-2.5 py-1.5 rounded-lg text-[10px] font-bold cursor-pointer hover:bg-blue-100 active:scale-95 transition whitespace-nowrap"
//                                     >
//                                         {isUploading ? <Loader2 size={12} className="animate-spin inline mr-1" /> : "Choose File"}
//                                     </label>

//                                     {/* অল্টারনেটিভ ইউআরএল ইনপুট ফিল্ড */}
//                                     <input
//                                         type="text"
...
//                             {/* 🖼️ ফেসবুক স্টাইল ইমেজ গ্রিড লেআউট */}
//                             {post.images && post.images.length > 0 && (
//                                 <div className={`grid gap-2 overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800 ${post.images.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>
//                                     {post.images.map((img, i) => (
//                                         <img key={i} src={img} alt="Post Content" className="w-full max-h-[300px] object-cover hover:scale-[1.02] transition-all duration-300" />
                    ...
                    {posts.map((post) => (
                        <div key={post.id} className="bg-white dark:bg-slate-900 p-4 md:p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                {/* 🔒 পোস্টের লেখক এবং কারেন্ট ইউজার এক হলে কেবল বাটন দেখাবে */}
                            ...
                            {/* মেইন বডি কন্টেন্ট (ইনলাইন এডিটিং সাপোর্টসহ) */}
                            {editingPostId === post.id ? (
                                <div className="space-y-2 bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                                    <textarea
                                        ...
                                        className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none min-h-[60px]"
                                    />
